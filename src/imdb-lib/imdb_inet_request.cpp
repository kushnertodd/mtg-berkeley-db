...
Imdb_inet_request::Imdb_inet_request(json_object *jobj, Bdb_errors &errors) {
  from_json(jobj, errors);
}

/*
  {
    "class_name": "Imdb_inet_request",
    "request": "imdb_load",
    "arguments": [
 ...
 */
void Imdb_inet_request::from_json(json_object *jobj, Bdb_errors &errors) {
  // parse: ' { "class_name": ... `
  std::string jobj_class_name =
      Bdb_json_utils::get_json_string("Imdb_inet_request::from_json", "1", jobj, "class_name", errors);
  if (!errors.has() && jobj_class_name != class_name())
    errors.add("Imdb_inet_request::from_json", "1", "not class Imdb_inet_request");
  // parse: request": ...
  request = Bdb_json_utils::get_json_string("Imdb_inet_request::from_json", "3", jobj, "request", errors);
  if (!errors.has()) {
    // parse: ' { "arguments": ... `
    json_object *arguments_json =
        Bdb_json_utils::get_json_object("Imdb_inet_request::from_json", "2", jobj, "arguments", json_type_array,
                                        errors);
    if (!errors.has()) {
      ...
      for (size_t i = 0; i < n_args && !errors.has(); i++) {
        ...
        std::string
            argument = Bdb_json_utils::get_json_string_type("Imdb_inet_request::from_json", "3", argument_json,
                                                            errors);
        if (!errors.has())
...
json_object *Imdb_inet_request::process_load_name_request(Imdb_inet_app_init &imdb_inet_app_init, Bdb_errors &errors) {
  if (arguments.empty())
    errors.add("Imdb_inet_request::process_load_name_request", "1", "missing imdb load name request arguments");
  Primary_database_config name_primary_database_config;
  imdb_inet_app_init.bdb_databases_config.select("name", name_primary_database_config, errors);
  if (!errors.has()) {
    std::unique_ptr<Bdb_key_extractor> imdb_bdb_key_extractor =
        std::make_unique<Imdb_bdb_key_extractor>();
    Primary_database
        name_db(name_primary_database_config, imdb_bdb_key_extractor.get(), imdb_inet_app_init.db_home, errors);
    if (!errors.has()) {
      Primary_database_config name_tripthongs_primary_database_config;
      imdb_inet_app_init.bdb_databases_config.select("name_tripthongs",
      ...
      if (!errors.has()) {
        Primary_database name_tripthongs_db(name_tripthongs_primary_database_config,
                                            ...
                                            imdb_inet_app_init.db_home,
                                            errors);
        if (!errors.has()) {
          std::string text_file = imdb_inet_app_init.tsv_home + "/" + arguments.at(0);
          Timer timer;
...
json_object *Imdb_inet_request::process_load_principals_request(Imdb_inet_app_init &imdb_inet_app_init,
                                                                Bdb_errors &errors) {
  if (arguments.empty())
    errors.add("Imdb_inet_request::process_load_principals_request", "1",
               "missing imdb load principals request arguments");
  Primary_database_config primary_database_config;
  imdb_inet_app_init.bdb_databases_config.select("principals", primary_database_config, errors);
  if (!errors.has()) {
    std::unique_ptr<Bdb_key_extractor> imdb_bdb_key_extractor =
        std::make_unique<Imdb_bdb_key_extractor>();
    Primary_database
        principals_db(primary_database_config, imdb_bdb_key_extractor.get(), imdb_inet_app_init.db_home, errors);
    if (!errors.has()) {
      std::string text_file = imdb_inet_app_init.tsv_home + "/" + arguments.at(0);
      Timer timer;
...
json_object *Imdb_inet_request::process_load_ratings_request(Imdb_inet_app_init &imdb_inet_app_init,
                                                             Bdb_errors &errors) {
  if (arguments.empty())
    errors.add("Imdb_inet_request::process_load_ratings_request", "1", "missing ratings_id");
  Primary_database_config primary_database_config;
  imdb_inet_app_init.bdb_databases_config.select("ratings", primary_database_config, errors);
  if (!errors.has()) {
    std::unique_ptr<Bdb_key_extractor> imdb_bdb_key_extractor =
        std::make_unique<Imdb_bdb_key_extractor>();
    Primary_database ratings_db(primary_database_config, imdb_bdb_key_extractor.get(), imdb_inet_app_init.db_home,
                                errors);
    if (!errors.has()) {
      std::string text_file = imdb_inet_app_init.tsv_home + "/" + arguments.at(0);
      Timer timer;
...
json_object *Imdb_inet_request::process_load_title_request(Imdb_inet_app_init &imdb_inet_app_init, Bdb_errors &errors) {
  if (arguments.empty())
    errors.add("Imdb_inet_request::process_load_title_request", "1", "missing title_id");
  Primary_database_config title_primary_database_config;
  imdb_inet_app_init.bdb_databases_config.select("title", title_primary_database_config, errors);
  if (!errors.has()) {
    std::unique_ptr<Bdb_key_extractor> imdb_bdb_key_extractor =
        std::make_unique<Imdb_bdb_key_extractor>();
    Primary_database
        title_db(title_primary_database_config, imdb_bdb_key_extractor.get(), imdb_inet_app_init.db_home, errors);
    if (!errors.has()) {
      Primary_database_config title_tripthongs_primary_database_config;
      imdb_inet_app_init.bdb_databases_config.select("title_tripthongs",
...
json_object *Imdb_inet_request::process_lookup_name_request(
    Imdb_inet_app_init &imdb_inet_app_init, Bdb_errors &errors) {
  if (arguments.empty())
    errors.add("Imdb_inet_request::process_lookup_name_request", "1", "missing name_id");
  Primary_database_config primary_database_config;
  imdb_inet_app_init.bdb_databases_config.select("name", primary_database_config, errors);
  if (!errors.has()) {
    std::unique_ptr<Bdb_key_extractor> imdb_bdb_key_extractor =
        std::make_unique<Imdb_bdb_key_extractor>();
    Primary_database name_db(primary_database_config, imdb_bdb_key_extractor.get(), imdb_inet_app_init.db_home, errors);
    if (!errors.has()) {
...
json_object *Imdb_inet_request::process_lookup_ratings_request(
    Imdb_inet_app_init &imdb_inet_app_init, Bdb_errors &errors) {
  if (arguments.empty())
    errors.add("Imdb_inet_request::run", "2", "missing ratings_id");
  Primary_database_config primary_database_config;
  imdb_inet_app_init.bdb_databases_config.select("ratings", primary_database_config, errors);
  if (!errors.has()) {
    std::unique_ptr<Bdb_key_extractor> imdb_bdb_key_extractor =
...
json_object *Imdb_inet_request::process_lookup_title_request(
    Imdb_inet_app_init &imdb_inet_app_init, Bdb_errors &errors) {
  if (arguments.empty())
    errors.add("Imdb_inet_request::process_lookup_title_request", "1", "missing title_id");
  std::unique_ptr<Bdb_key_extractor> imdb_bdb_key_extractor =
      std::make_unique<Imdb_bdb_key_extractor>();
  Primary_database_config primary_title_database_config;
  imdb_inet_app_init.bdb_databases_config.select("title", primary_title_database_config, errors);
  if (!errors.has()) {
    Primary_database_config primary_ratings_database_config;
    imdb_inet_app_init.bdb_databases_config.select("ratings", primary_ratings_database_config, errors);
    if (!errors.has()) {
      Primary_database
          title_db(primary_title_database_config, imdb_bdb_key_extractor.get(), imdb_inet_app_init.db_home,
                   errors);
      Primary_database
          ratings_db(primary_ratings_database_config, imdb_bdb_key_extractor.get(), imdb_inet_app_init.db_home,
                     errors);
      if (!errors.has()) {
...
json_object *Imdb_inet_request::process_request(
    Imdb_inet_app_init &imdb_inet_app_init,
    const std::string &request,
    Bdb_errors &errors) {
  Imdb_request_response imdb_request_response(errors);
  json_object *request_json = json_tokener_parse(request.c_str());
  if (request_json == nullptr)
    errors.add("Imdb_inet_request::process_request",
               "1",
  ...
  if (!errors.has()) {
    ...
    request_class_name =
        Bdb_json_utils::get_json_string("Imdb_inet_request::process_request", "2",
                                        request_json, "class_name",
                                        errors);
  }
  if (!errors.has()) {
    if (request_class_name == Imdb_inet_request::class_name()) {
      Imdb_inet_request imdb_inet_request(request_json, errors);
      if (!errors.has()) {
        json_object *request_response_json = imdb_inet_request.run(imdb_inet_app_init, errors);
      ...
      if (!errors.has())
        for (Imdb_inet_request &imdb_inet_request: imdb_requests.request_list) {
          json_object *request_response_json =
              imdb_inet_request.run(imdb_inet_app_init, errors);
          imdb_request_response.add_response_array(request_response_json);
        }
    } else
      errors.add("Imdb_inet_request::process_request", "3", "invalid request class: " + request_class_name);
  }
  if (errors.has()) {
...
json_object *Imdb_inet_request::process_search_name_request(
    Imdb_inet_app_init &imdb_inet_app_init, Bdb_errors &errors) {
  if (arguments.empty())
    errors.add("Imdb_inet_request::process_search_name_request", "1", "missing name_id");
  Primary_database_config principals_primary_database_config;
  imdb_inet_app_init.bdb_databases_config.select("principals", principals_primary_database_config, errors);
  ...
  if (!errors.has()) {
    std::unique_ptr<Bdb_key_extractor> imdb_bdb_key_extractor =
        std::make_unique<Imdb_bdb_key_extractor>();
    Primary_database name_db(name_primary_database_config, imdb_bdb_key_extractor.get(), imdb_inet_app_init.db_home,
                             errors);
    Primary_database
        principals_db(principals_primary_database_config, imdb_bdb_key_extractor.get(), imdb_inet_app_init.db_home,
                      errors);
    Secondary_database
        principals_title_id_sdb(principals_title_id_secondary_database_config, imdb_inet_app_init.db_home, errors);
    if (!errors.has()) {
      std::string title_id = arguments.at(0);
...
json_object *Imdb_inet_request::process_search_name_tripthongs_request(
    Imdb_inet_app_init &imdb_inet_app_init, Bdb_errors &errors) {
  if (arguments.size() < 3)
    errors.add("Imdb_inet_request::process_search_name_tripthongs_request",
               "1",
...
json_object *Imdb_inet_request::process_search_title_request(
    Imdb_inet_app_init &imdb_inet_app_init, Bdb_errors &errors) {
  if (arguments.empty())
    errors.add("Imdb_inet_request::process_search_title_request", "1", "missing name_id");
  Primary_database_config principals_primary_database_config;
  imdb_inet_app_init.bdb_databases_config.select("principals", principals_primary_database_config, errors);
  ...
  imdb_inet_app_init.bdb_databases_config.select("ratings", ratings_primary_database_config, errors);
  if (!errors.has()) {
    std::unique_ptr<Bdb_key_extractor> imdb_bdb_key_extractor =
        std::make_unique<Imdb_bdb_key_extractor>();
    Primary_database
        title_db(title_primary_database_config, imdb_bdb_key_extractor.get(), imdb_inet_app_init.db_home, errors);
    Primary_database
        ratings_db(ratings_primary_database_config, imdb_bdb_key_extractor.get(), imdb_inet_app_init.db_home, errors);
    Primary_database
        principals_db(principals_primary_database_config, imdb_bdb_key_extractor.get(), imdb_inet_app_init.db_home,
                      errors);
    Secondary_database
        principals_name_id_sdb(principals_name_id_secondary_database_config, imdb_inet_app_init.db_home, errors);
    if (!errors.has()) {
      std::string name_id = arguments.at(0);
...
json_object *Imdb_inet_request::process_search_title_tripthongs_request(
    Imdb_inet_app_init &imdb_inet_app_init, Bdb_errors &errors) {
  if (arguments.size() < 3)
    errors.add("Imdb_inet_request::process_search_title_tripthongs_request",
               "1",
               "parameters: text, min-score, score-count");
  std::string text;
  int min_score;
...
json_object *Imdb_inet_request::process_select_name_request(Imdb_inet_app_init &imdb_inet_app_init,
                                                            Bdb_errors &errors) {
  return process_search_name_tripthongs_request(imdb_inet_app_init, errors);
}

json_object *Imdb_inet_request::process_select_principals_request(Imdb_inet_app_init &imdb_inet_app_init,
                                                                  Bdb_errors &errors) {
  Primary_database_config primary_database_config;
  imdb_inet_app_init.bdb_databases_config.select("principals", primary_database_config, errors);
  if (!errors.has()) {
...
json_object *Imdb_inet_request::process_select_ratings_request(Imdb_inet_app_init &imdb_inet_app_init,
                                                               Bdb_errors &errors) {
  Primary_database_config primary_database_config;
  imdb_inet_app_init.bdb_databases_config.select("ratings", primary_database_config, errors);
  if (!errors.has()) {
...
json_object *Imdb_inet_request::process_select_title_request(Imdb_inet_app_init &imdb_inet_app_init,
                                                             Bdb_errors &errors) {
  return process_search_title_tripthongs_request(imdb_inet_app_init, errors);
}

json_object *Imdb_inet_request::process_update_name_request(Imdb_inet_app_init &imdb_inet_app_init,
                                                            Bdb_errors &errors) {
  if (arguments.size() < 3)
    errors.add("Imdb_inet_request::process_update_name_request", "1", "missing name_id");
  Primary_database_config primary_database_config;
  imdb_inet_app_init.bdb_databases_config.select("name", primary_database_config, errors);
  if (!errors.has()) {
    std::unique_ptr<Bdb_key_extractor> imdb_bdb_key_extractor =
...
json_object *Imdb_inet_request::process_update_title_request(Imdb_inet_app_init &imdb_inet_app_init,
                                                             Bdb_errors &errors) {
  if (arguments.size() < 3)
    errors.add("Imdb_inet_request::process_update_name_request", "1", "missing title_id");
  std::unique_ptr<Bdb_key_extractor> imdb_bdb_key_extractor =
      std::make_unique<Imdb_bdb_key_extractor>();
  Primary_database_config primary_title_database_config;
  imdb_inet_app_init.bdb_databases_config.select("title", primary_title_database_config, errors);
  if (!errors.has()) {
    ...
    if (!errors.has()) {
      Primary_database
          title_db(primary_title_database_config, imdb_bdb_key_extractor.get(), imdb_inet_app_init.db_home, errors);
      Primary_database
          ratings_db(primary_ratings_database_config, imdb_bdb_key_extractor.get(), imdb_inet_app_init.db_home,
                     errors);
      if (!errors.has()) {
...
json_object *Imdb_inet_request::run(Imdb_inet_app_init &imdb_inet_app_init,
                                    Bdb_errors &errors) {
  json_object *request_response = nullptr;
  if (request == "load_name") {
    request_response = process_load_name_request(imdb_inet_app_init, errors);
  } else if (request == "load_principals") {
    request_response = process_load_principals_request(imdb_inet_app_init, errors);
  } else if (request == "load_ratings") {
    request_response = process_load_ratings_request(imdb_inet_app_init, errors);
  } else if (request == "load_title") {
    request_response = process_load_title_request(imdb_inet_app_init, errors);
  } else if (request == "lookup_name") {
    request_response = process_lookup_name_request(imdb_inet_app_init, errors);
  } else if (request == "lookup_ratings") {
    request_response = process_lookup_ratings_request(imdb_inet_app_init, errors);
  } else if (request == "lookup_title") {
    request_response = process_lookup_title_request(imdb_inet_app_init, errors);
  } else if (request == "search_name") {
    request_response = process_search_name_request(imdb_inet_app_init, errors);
  } else if (request == "search_name_tripthongs") {
    request_response = process_search_name_tripthongs_request(imdb_inet_app_init, errors);
  } else if (request == "search_title") {
    request_response = process_search_title_request(imdb_inet_app_init, errors);
  } else if (request == "search_title_tripthongs") {
    request_response = process_search_title_tripthongs_request(imdb_inet_app_init, errors);
  } else if (request == "select_name") {
    request_response = process_select_name_request(imdb_inet_app_init, errors);
  } else if (request == "select_principals") {
    request_response = process_select_principals_request(imdb_inet_app_init, errors);
  } else if (request == "select_ratings") {
    request_response = process_select_ratings_request(imdb_inet_app_init, errors);
  } else if (request == "select_title") {
    request_response = process_select_title_request(imdb_inet_app_init, errors);
  } else if (request == "update_name") {
    request_response = process_update_name_request(imdb_inet_app_init, errors);
  } else if (request == "update_title") {
    request_response = process_update_title_request(imdb_inet_app_init, errors);
  } else
    errors.add("Imdb_inet_request::run", "1", "unrecognized request " + request);
  if (!errors.has())
...
json_object *Imdb_inet_request::to_json(Bdb_errors &errors) {
  json_object *root = json_object_new_object();
  if (!root) {
    errors.add("Imdb_inet_request::to_json", "1", "json-c allocate error");
    return nullptr;
  }
...
std::string Imdb_inet_request::to_string() const {
  std::ostringstream os;
  os << "imdb_inet_request:" << std::endl;
...
json_object *Imdb_requests::to_json(Bdb_errors &errors) {
  ...
  if (!errors.has()) {
    ...
    for (auto &imdb_inet_request: request_list) {
      json_object *request_json = imdb_inet_request.to_json(errors);
      if (errors.has())
        break;
