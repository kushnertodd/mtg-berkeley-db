  ...
  while ((ch = getopt(argc, (char **) argv, ":b:d:g:h:p:t:")) != EOF)
    switch (ch) {
      ...
      case '?':
        ...
        break;
      default:
        errors.add("Mtg_inet_app_init::init", "1", "Invalid option '-" + Misc_utils::char_to_string((char) ch));
    }
...
void Mtg_inet_app_init::init(Bdb_errors &errors) {
...
void Mtg_inet_app_init::usage(const std::string &message) {
