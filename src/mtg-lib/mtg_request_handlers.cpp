...
bool Mtg_card_request_handler::select_all_decks(Mtg_inet_app_init &mtg_inet_app_init,
  ...
  if (!errors.has()) {
    std::unique_ptr<Bdb_key_extractor> mtg_bdb_key_extractor =
        std::make_unique<Mtg_bdb_key_extractor>();
    Primary_database
        deck_db(deck_primary_database_config, mtg_bdb_key_extractor.get(), mtg_inet_app_init.db_home,
                errors);
    Primary_database
...
bool Mtg_card_request_handler::select_all_for_name(Mtg_inet_app_init &mtg_inet_app_init,
  ...
  if (mtg_request.request != "card_select_all_for_name")
    return false;
  if (mtg_request.arguments.empty())
    errors.add("Mtg_card_request_handler::card_select_all_for_name",
               "1",
               "parameters: name");
  std::string name;
  if (!errors.has()) {
    ...
    Secondary_database_config card_name_secondary_database_config;
    if (!errors.has())
      card_primary_database_config.select("card_name",
                                          card_name_secondary_database_config,
                                          errors);
    if (!errors.has()) {
...
bool Mtg_card_request_handler::select_all_for_type_id(Mtg_inet_app_init &mtg_inet_app_init,
  ...
  if (mtg_request.request != "card_select_all_for_type_id")
    return false;
  if (mtg_request.arguments.empty())
    errors.add("Mtg_card_request_handler::card_select_all_for_type_id",
               "1",
               "parameters: type_id");
  std::string type_id;
  if (!errors.has()) {
    ...
    if (!errors.has()) {
      ...
      if (!errors.has()) {
        ...
        if (!errors.has()) {
          ...
          Card_DAO::select_all_for_type_id(card_type_id_sdb.bdb_db,
                                           card_db.bdb_db,
                                           type_id,
                                           card_dto_list,
                                           errors);
...
bool Mtg_deck_request_handler::handle(Mtg_inet_app_init &mtg_inet_app_init,
                                      ...
                                      Mtg_request_response &mtg_request_response,
                                      Bdb_errors &errors) {
  if (!Mtg_deck_request_handler::add_card(mtg_inet_app_init, mtg_request, mtg_request_response, errors)
...
bool Mtg_deck_request_handler::add_card(Mtg_inet_app_init &mtg_inet_app_init,
                                        const Mtg_request &mtg_request,
                                        Mtg_request_response &mtg_request_response,
                                        Bdb_errors &errors) {
  if (mtg_request.request != "deck_add_card")
    return false;
  if (mtg_request.arguments.size() < 2)
    errors.add("Mtg_card_request_handler::add_card", "1", "missing deck_id or card_id");
  Primary_database_config deck_card_primary_database_config;
  mtg_inet_app_init.bdb_databases_config.select("deck_card", deck_card_primary_database_config, errors);
...
bool Mtg_deck_request_handler::select_all_cards(Mtg_inet_app_init &mtg_inet_app_init,
  ...
  if (!errors.has()) {
    std::string deck_id = mtg_request.arguments.at(0);
    Card_DTO_list card_dto_list;
    Deck_card_DAO::select_cards_for_deck(deck_card_deck_id_sdb.bdb_db,
                                         deck_card_db.bdb_db,
                                         card_db.bdb_db,
                                         deck_id,
                                         card_dto_list,
...
bool Mtg_deck_request_handler::select_all_for_account_id(Mtg_inet_app_init &mtg_inet_app_init,
  ...
  if (mtg_request.request != "deck_select_all_for_account_id")
    return false;
  if (mtg_request.arguments.empty())
    errors.add("Mtg_deck_request_handler::select_all_for_account_id",
               "1",
    ...
    if (!errors.has()) {
      ...
      if (!errors.has()) {
        ...
        if (!errors.has()) {
          ...
          Deck_DAO::select_all_for_account_id(deck_account_id_sdb.bdb_db,
                                              deck_db.bdb_db,
                                              account_id,
                                              deck_dto_list,
                                              errors);
...
bool Mtg_deck_request_handler::select_all_for_name(Mtg_inet_app_init &mtg_inet_app_init,
  ...
  if (mtg_request.request != "deck_select_all_for_name")
    return false;
  if (mtg_request.arguments.empty())
    errors.add("Mtg_deck_request_handler::select_all_for_name",
               "1",
               "parameters: name");
  std::string name;
  if (!errors.has()) {
    ...
    Primary_database_config deck_primary_database_config;
    mtg_inet_app_init.bdb_databases_config.select("deck", deck_primary_database_config, errors);
    Secondary_database_config deck_name_secondary_database_config;
    if (!errors.has())
      deck_primary_database_config.select("deck_name",
                                          deck_name_secondary_database_config,
                                          errors);
    if (!errors.has()) {
      Primary_database deck_db(deck_primary_database_config,
                               mtg_bdb_key_extractor.get(),
